...
SET PB_DDK_DIR="C:\Program Files (x86)\Windows Kits\10\Include\10.0.14393.0"
...
IF DEFINED PB_CERT (
  ...
  FOR %%F IN (
    "bin14\Win32\Release" "bin14\Win32\Release_(Vista)" "bin14\x64\Release" "bin14\x64\Release_(Vista)"
  ) DO (
    PUSHD %%F
...
"%InnoSetupPath%\iscc.exe" /SStandard="cmd /c "..\..\..\bin\windows\sign_driver.cmd" $f "^
 /Q /O"..\..\..\distribution" "setup.iss" /DVS2015build
...
:SubMSVC
TITLE Compiling PeerBlock with MSVC 2015 - %~1^|%~2...
...
START "" /B /WAIT "..\..\..\bin\windows\7za.exe" a -tzip -mx=9^
 "PeerBlock_r%buildnum%__%1_%~2_VS2015.zip" "peerblock.exe" "pbfilter.sys"^
 "license.txt" "readme.rtf" >NUL
IF %ERRORLEVEL% NEQ 0 GOTO ErrorDetected

ECHO PeerBlock_r%buildnum%__%1_%~2_VS2015.zip created successfully!
MOVE /Y "PeerBlock_r%buildnum%__%1_%~2_VS2015.zip" "..\..\..\distribution" >NUL
ECHO.
POPD
